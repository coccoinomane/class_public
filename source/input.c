  ...
  */
  ...
  if ((flag1 == _TRUE_) && ((strstr(string1,"y") != NULL) || (strstr(string1,"Y") != NULL))) {
    ...
    if ((ppt->has_scalars == _TRUE_) &&
        ...
        (ppt->has_cl_cmb_lensing_potential == _TRUE_) || (pbi->has_isw_lensing == _TRUE_)) {
  ...
  if ((ppt->has_cls == _TRUE_) || (pbi->has_bispectra == _TRUE_)) {
    ...
    if (ppt->has_vectors == _TRUE_)
      pbs->l_max=MAX(ppt->l_vector_max,pbs->l_max);
...
int input_try_unknown_parameters(double * unknown_parameter,
  ...
  if (pfzw->required_computation_stage >= cs_bessels){
    ...
    class_call(bessel_init(&pr,&tr,&bs),bs.error_message, errmsg);
  }
  ...
  if (pfzw->required_computation_stage >= cs_bispectra){
    class_call(bispectra_free(&pt,&sp,&bi), bi.error_message, errmsg);
  }
  if (pfzw->required_computation_stage >= cs_bessels){
